...
export default function MathematicsFacultyPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const [profileRes, facultyRes, coursesRes, experimentsRes, assignmentsRes, liveClassesRes, studentsRes] = await Promise.all([
                ...
                supabase.from('courses').select('*, lessons(count), enrollments(count)').ilike('subject', 'mathematics').order('created_at', { ascending: false }),
                supabase.from('experiments').select('*').ilike('subject', 'mathematics').order('created_at', { ascending: false }).limit(6),
                supabase.from('assignments').select('*, submissions(count)').ilike('title', '%Math%').order('due_date', { ascending: true }).limit(5),
                supabase.from('live_classes').select('*').ilike('title', '%Math%').gte('start_time', new Date().toISOString()).order('start_time', { ascending: true }).limit(3),
    ...
    return (
        <FacultySubjectDashboard
            subject="mathematics" subjectName="Mathematics" icon="📐" gradient="from-purple-500 to-pink-400"

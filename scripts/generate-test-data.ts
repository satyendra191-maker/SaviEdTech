 ...
 * Or: node src/scripts/generate-test-data.js (after building)
 * 
 * Generates:
 * - 10 students
 * - 10 parents
...
const EXAM_TARGETS = ['JEE', 'NEET', 'JEE+NEET'];
...
async function main() {
    ...
    for (const faculty of FACULTY_DATA) {
        const { data: authData, error: authError } = await supabase.auth.admin.createUser({
            ...
            password: 'Test@123',
            email_confirm: true,
            user_metadata: { full_name: faculty.name, subject: faculty.subject },
        });
